...
import os
os.environ["TRACKER_FILE"] = "posted_topics_hindi.json"
os.environ["LANGUAGE"] = "hindi"
...
def get_past_urls(count: int = 5) -> list:
    ...
    try:
        tracker_filename = os.getenv("TRACKER_FILE", "posted_topics.json")
        tracker_file = Path(__file__).parent.parent / "data" / tracker_filename
...
def run_batch():
    ...
    for i, topic in enumerate(topics_to_publish, 1):
        ...
        try:
            ...
            # Generate article
            article = content_generator.generate_article(topic, past_urls=past_urls)
            ...
            try:
                ...
                final_html = content_generator.inject_image(article["body_html"], "")
